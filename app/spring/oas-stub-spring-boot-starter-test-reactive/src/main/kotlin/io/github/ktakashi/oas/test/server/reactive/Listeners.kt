...
class OasStubReactiveServerApplicationListener : ApplicationListener<ApplicationPreparedEvent> {
    ...
    private fun registerPorts(environment: ConfigurableEnvironment) {
        val httpPortProp = "${OAS_STUB_REACTIVE_SERVER_PROPERTY_PREFIX}.port"
        ...
        val httpsPortProp = "${OAS_STUB_REACTIVE_SERVER_PROPERTY_PREFIX}.https-port"
    ...
    private fun getOasStubServerSource(environment: ConfigurableEnvironment): MutableMap<String, Any> {
        ...
        if (propertySources.contains(OAS_STUB_REACTIVE_SERVER_PROPERTY_PREFIX)) {
            propertySources.remove(OAS_STUB_SERVER_BEAN_NAME)?.let {
                propertySources.addFirst(it)
            }
        } else {
            propertySources.addFirst(MapPropertySource(OAS_STUB_REACTIVE_SERVER_PROPERTY_PREFIX, mutableMapOf()))
        }
        return propertySources[OAS_STUB_REACTIVE_SERVER_PROPERTY_PREFIX]!!.source as MutableMap<String, Any>
    }

}

class OasStubReactiveServerTestExecutionListener: AbstractTestExecutionListener() {
    ...
    private fun oasStubServerConfiguration(testContext: TestContext) = testContext.applicationContext.getBean(OasStubReactiveConfiguration::class.java)
    ...
    private fun configurationMissing(testContext: TestContext) = !testContext.applicationContext.containsBean(OAS_STUB_REACTIVE_CONFIGURATION_BEAN_NAME)

    private fun annotationMissing(testContext: TestContext) = !TestContextAnnotationUtils.hasAnnotation(testContext.testClass, AutoConfigureOasStubReactiveServer::class.java)
